...
import { PLATFORMS, CREATOR_TYPES } from "@/lib/types"
...
function extractTikTokUsername(input: string): string | null {
  ...
  // Match @username or tiktok.com/@username or tiktok.com/@username/...
  const urlMatch = trimmed.match(/tiktok\.com\/@([^/?&#\s]+)/i)
  if (urlMatch) return urlMatch[1]
  // Plain @username
  if (trimmed.startsWith("@")) return trimmed.slice(1)
  // Plain username (no spaces, no slashes)
...
export default function ApplicationForm() {
  ...
  const handleProfileLinkChange = (value: string) => {
    ...
    // Reset lookup state when link changes
    if (tiktokData) {
      setTiktokData(null)
  ...
  const canAnalyze = form.profile_link.trim().length > 0 && !lookupLoading

  // Valid if tiktokData filled the view fields, or user manually selected them
  const isValid =
    form.email.trim() &&
    form.platform &&
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-xl mx-auto px-6 py-12">
        ...
        <div className="space-y-8">
          ...
          {/* Profile link + Analyze */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">
              TikTok Profile Link <span className="text-red-400">*</span>
            </label>
          ...
          {/* Hint when stats not yet loaded */}
          {!tiktokData && (
            <div className="text-xs text-gray-400 bg-gray-50 rounded-xl px-4 py-3">
              Enter your TikTok link above and click <strong>Analyze</strong> — we'll fetch your stats automatically so you don't have to fill them in manually.
            </div>
          )}
        </div>
